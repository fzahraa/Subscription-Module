...
const AddProject = ({ handleStep }) => {
    ...
    var projectLimit = [];
if(subs!=null){
    projectLimit = subs.data.filter((data)=> data._id === user.profile.subscriptionPackage);
    console.log(projectLimit[0].projectLimit);
}
var currentProjects = 0;
if(user!=null){
currentProjects =  user.profile.portfolio.length;
}
    useEffect(() => {
        if (isSuccess) {
            swal({
    ...
    const handleSubmit = (e) => {
        ...
        if(projectLimit[0].projectLimit >= (currentProjects + 1)){
        //  API CALL.
        dispatch(
            addProjectEn({
                id: user.profile._id,
        ...
        alert("You can't add moe projects. Please upgrade your subscription model!")
    }
        // Reset form.
        setProjectName("");
        setProjectLocation("");
